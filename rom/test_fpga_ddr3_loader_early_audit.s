...
calib_done:
    ...
    li x10, LOADER_EA_EVT_IDLE_OK
    jal ra, emit_event
    jal ra, recv_training_window
    jal ra, flush_uart_to_header
    li x10, LOADER_EA_EVT_HEADER_ENTER
    jal ra, emit_event

    li x23, 0                       # bad magic byte index
    li x24, BMK1_MAGIC_B0
    li x11, LOADER_EA_EVT_HDR_B0_RX
    jal ra, recv_magic_byte
    li x23, 1
    li x24, BMK1_MAGIC_B1
    li x11, LOADER_EA_EVT_HDR_B1_RX
    jal ra, recv_magic_byte
    li x23, 2
    li x24, BMK1_MAGIC_B2
    li x11, LOADER_EA_EVT_HDR_B2_RX
    jal ra, recv_magic_byte
    li x23, 3
    li x24, BMK1_MAGIC_B3
    li x11, LOADER_EA_EVT_HDR_B3_RX
    jal ra, recv_magic_byte
...
recv_training_window:
    mv x12, ra
    li x10, LOADER_EA_EVT_TRAIN_START
    jal ra, emit_event
...
recv_training_window_non55:
    ...
    andi x7, x17, 0xFF
    slli x10, x7, 8
    ori x10, x10, LOADER_EA_EVT_TRAIN_DONE
    jal ra, emit_event
    mv ra, x12
    jalr x0, 0(ra)
...
flush_uart_quiet:
    ...
    andi x7, x17, 0xFF
    slli x10, x7, 8
    ori x10, x10, LOADER_EA_EVT_FLUSH_DONE
    jal ra, emit_event
    mv ra, x12
    jalr x0, 0(ra)
