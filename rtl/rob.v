...
//   Single-thread version (SMT support removed).
//   - Depth: 32 entries
...
module rob #(
    parameter ROB_DEPTH     = 32,       // Entries (power of 2)
...
reg  [PHYS_REG_W-1:0]  rob_prd_new    [0:ROB_DEPTH-1];
reg  [PHYS_REG_W-1:0]  rob_prd_old    [0:ROB_DEPTH-1];
reg                     rob_is_branch  [0:ROB_DEPTH-1];
reg                     rob_regs_write [0:ROB_DEPTH-1];
...
always @(*) begin
    ...
    if (free_query0_prd != {PHYS_REG_W{1'b0}}) begin
        for (free_q0_idx = 0; free_q0_idx < ROB_DEPTH; free_q0_idx = free_q0_idx + 1) begin
            if (rob_valid[free_q0_idx] &&
                rob_regs_write[free_q0_idx] &&
                (rob_prd_old[free_q0_idx] == free_query0_prd))
...
always @(*) begin
    ...
    if (free_query1_prd != {PHYS_REG_W{1'b0}}) begin
        for (free_q1_idx = 0; free_q1_idx < ROB_DEPTH; free_q1_idx = free_q1_idx + 1) begin
            if (rob_valid[free_q1_idx] &&
                rob_regs_write[free_q1_idx] &&
                (rob_prd_old[free_q1_idx] == free_query1_prd))
...
wire rob_full_t0 = (rob_count >= ROB_DEPTH - 2);

assign rob0_full = rob_full_t0 || recovering_r;
assign rob1_full = rob_full_t0 || recovering_r;

// =============================================================================
// Dispatch Allocation -- ROB Index Output
// =============================================================================

// disp0 gets current tail
assign disp0_rob_idx = rob_tail;
// disp1 gets next slot (if disp0 is valid, +1)
assign disp1_rob_idx = rob_tail + (disp0_valid ? {{(ROB_IDX_W-1){1'b0}}, 1'b1} : {ROB_IDX_W{1'b0}});
...
wire wb0_completes_head_t0 = wb0_valid &&
    rob_valid[rob_head] && !rob_complete[rob_head] &&
    (rob_tag[rob_head] == wb0_tag);
wire wb1_completes_head_t0 = wb1_valid &&
    rob_valid[rob_head] && !rob_complete[rob_head] &&
    (rob_tag[rob_head] == wb1_tag);
wire head_complete_now_t0 = rob_complete[rob_head] ||
    wb0_completes_head_t0 || wb1_completes_head_t0;
wire head_has_result_t0 = rob_has_result[rob_head] ||
    (wb0_completes_head_t0 && wb0_regs_write && (rob_rd[rob_head] != 5'd0)) ||
    (wb1_completes_head_t0 && wb1_regs_write && (rob_rd[rob_head] != 5'd0));
wire [31:0] head_result_t0 = rob_has_result[rob_head] ? rob_result[rob_head] :
    wb0_completes_head_t0 ? wb0_data :
    wb1_completes_head_t0 ? wb1_data : 32'd0;

wire commit_flush_blocks_t0 =
    flush && rob_valid[rob_head] &&
    (!flush_order_valid || (rob_order_id[rob_head] > flush_order_id));
...
assign head_valid_t0       = rob_valid[rob_head];
assign head_order_id_t0    = rob_order_id[rob_head];
assign head_flushed_t0     = rob_flushed[rob_head];
...
always @(posedge clk or negedge rstn) begin
    if (!rstn) begin
        ...
        for (j = 0; j < ROB_DEPTH; j = j + 1) begin
            ...
            rob_prd_old[j]    <= {PHYS_REG_W{1'b0}};
            rob_is_branch[j]  <= 1'b0;
            rob_regs_write[j] <= 1'b0;
            rob_pc[j]         <= 32'd0;
        end
    end else begin
        commit0_valid_r <= 1'b0;
        ...
        recover_en_r    <= 1'b0;
        debug_commit_suppressed_r <= 1'b0;
        if (commit_flush_blocks_t0 && head_complete_now_t0) begin
        ...
        if (recovering_r) begin
            if (rob_valid[recover_ptr_r] &&
                rob_flushed[recover_ptr_r]) begin
                // Output current (flushed) entry's recovery info
                recover_en_r         <= rob_regs_write[recover_ptr_r] && (rob_rd[recover_ptr_r] != 5'd0);
                recover_rd_r         <= rob_rd[recover_ptr_r];
                recover_prd_old_r    <= rob_prd_old[recover_ptr_r];
                recover_prd_new_r    <= rob_prd_new[recover_ptr_r];
                recover_regs_write_r <= rob_regs_write[recover_ptr_r];

                // Invalidate entry
                rob_valid[recover_ptr_r]   <= 1'b0;
                rob_flushed[recover_ptr_r] <= 1'b0;
        ...
        if (flush) begin
            ...
            if (!recovering_r && rob_count > 0) begin
                recovering_r  <= 1'b1;
                recover_ptr_r <= rob_tail - 1;
            end
        end
        ...
        if (!recovering_r) begin : t0_next_state
            ...
            if (next_count != {(ROB_IDX_W+1){1'b0}} && !commit_flush_blocks_t0) begin
                if (!rob_valid[rob_head]) begin
                ...
                end else if (rob_flushed[rob_head]) begin
                    // Skip flushed entry
                    rob_valid[rob_head]      <= 1'b0;
                    rob_has_result[rob_head] <= 1'b0;
                    rob_result[rob_head]     <= 32'd0;
                    next_head = rob_head + 1;
                    next_count = next_count - 1;
                end else if (head_complete_now_t0) begin
                    // Commit head entry
                    commit0_valid_r      <= 1'b1;
                    commit0_rd_r         <= rob_rd[rob_head];
                    commit0_tag_r        <= rob_tag[rob_head];
                    commit0_has_result_r <= head_has_result_t0;
                    commit0_data_r       <= head_result_t0;
                    commit0_order_id_r   <= rob_order_id[rob_head];
                    ...
                    commit0_regs_write_r <= rob_regs_write[rob_head];
                    rob_valid[rob_head]      <= 1'b0;
                    rob_has_result[rob_head] <= 1'b0;
                    rob_result[rob_head]     <= 32'd0;
                    next_head = rob_head + 1;
            ...
            // -- Dispatch Allocation --
            if (!flush && disp0_valid && !rob0_full) begin
                `ifdef VERBOSE_SIM_LOGS
                $display("[ROB DISP0] tail=%0d tag=%0d rd=%0d pc=%h @%0t",
                ...
                rob_prd_old[next_tail]    <= disp0_prd_old;
                rob_is_branch[next_tail]  <= disp0_is_branch;
                rob_regs_write[next_tail] <= disp0_regs_write;
                rob_pc[next_tail]         <= disp0_pc;
                next_tail = next_tail + 1;
                next_count = next_count + 1;
            end

            if (!flush && disp1_valid && !rob1_full) begin
                rob_valid[next_tail]      <= 1'b1;
                rob_complete[next_tail]   <= 1'b0;
                rob_flushed[next_tail]    <= 1'b0;
                ...
                rob_prd_old[next_tail]    <= disp1_prd_old;
                rob_is_branch[next_tail]  <= disp1_is_branch;
                rob_regs_write[next_tail] <= disp1_regs_write;
                rob_pc[next_tail]         <= disp1_pc;
                next_tail = next_tail + 1;
                next_count = next_count + 1;
            end
        ...
        // commit1 is never asserted in single-thread mode
        // (it was previously driven by T1's ROB which is now removed)
    end
end
