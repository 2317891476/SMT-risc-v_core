...
assign alu_op = (inst_op == `ItypeL || inst_op == `Stype || inst_op == `AMO_OP || inst_op == `UtypeL || inst_op == `UtypeU || inst_op == `MISC_MEM)? 3'b000 :
...
assign mem_write = (inst_op == `Stype || inst_op == `AMO_OP) ? 1'b1 : 1'b0; // for store/AMO
assign mem_read = (inst_op == `ItypeL || inst_op == `AMO_OP) ? 1'b1 : 1'b0; // for load/AMO
assign regs_write = (inst_op == `ItypeL || inst_op == `AMO_OP || inst_op == `Rtype || inst_op == `UtypeU || inst_op == `ItypeA || inst_op == `ItypeJ || inst_op == `UtypeL || inst_op == `Jtype || inst_op == `OPC_CUSTOM0)? 1:0;//for the inst that writes back the regs
//assign regs_write = (inst_op == `Stype || inst_op == `Btype)? 0:1;     // when the inst is beyond the definition ,like 0x00000000, reg_write is 0  
assign mem2reg = (regs_write && (inst_op == `ItypeL || inst_op == `AMO_OP))?1:0; //for load/AMO writeback
