`timescale 1ns/1ns
// =============================================================================
// Module : freelist
// Description:
//   Physical register free FIFO with dual allocate, dual commit free,
//   ROB-walk recovery fallback, and precise branch checkpoints.
//   Single-thread version.
// =============================================================================
`include "define.v"

module freelist #(
    parameter PHYS_REG_W    = 6,
    parameter NUM_FREE      = 32,
    parameter FL_DEPTH      = 64,
    parameter FL_IDX_W      = 6,
    parameter BR_CKPT_DEPTH = 32,
    parameter BR_CKPT_IDX_W = 5
)(
    input  wire        clk,
    input  wire        rstn,

    input  wire        alloc0_req,
    input  wire        alloc1_after_alloc0,
    output wire [PHYS_REG_W-1:0] alloc0_prd,
    input  wire        alloc1_req,
    output wire [PHYS_REG_W-1:0] alloc1_prd,
    output wire        can_alloc_1,
    output wire        can_alloc_2,

    input  wire        free0_valid,
    input  wire [PHYS_REG_W-1:0] free0_prd,
    input  wire        free1_valid,
    input  wire [PHYS_REG_W-1:0] free1_prd,

    input  wire        recover_push_valid,
    input  wire [PHYS_REG_W-1:0] recover_push_prd,

    input  wire        branch_ckpt_capture0_valid,
    input  wire [`METADATA_ORDER_ID_W-1:0] branch_ckpt_capture0_order_id,
    input  wire [1:0]  branch_ckpt_capture0_alloc_count,
    input  wire        branch_ckpt_capture1_valid,
    input  wire [`METADATA_ORDER_ID_W-1:0] branch_ckpt_capture1_order_id,
    input  wire [1:0]  branch_ckpt_capture1_alloc_count,
    input  wire        branch_ckpt_restore,
    input  wire [`METADATA_ORDER_ID_W-1:0] branch_ckpt_restore_order_id,
    output reg         branch_ckpt_restore_hit,
    input  wire        branch_ckpt_drop0_valid,
    input  wire [`METADATA_ORDER_ID_W-1:0] branch_ckpt_drop0_order_id,
    input  wire        branch_ckpt_drop1_valid,
    input  wire [`METADATA_ORDER_ID_W-1:0] branch_ckpt_drop1_order_id,

    input  wire        rebuild_valid,
    input  wire [FL_DEPTH-1:0] rebuild_mapped_mask,

    input  wire        reset_list
);

    reg [PHYS_REG_W-1:0] fl_mem  [0:FL_DEPTH-1];
    reg [FL_IDX_W:0]     fl_head;
    reg [FL_IDX_W:0]     fl_tail;

    reg                         ckpt_valid [0:BR_CKPT_DEPTH-1];
    reg [`METADATA_ORDER_ID_W-1:0] ckpt_order_id [0:BR_CKPT_DEPTH-1];
    reg [PHYS_REG_W-1:0]        ckpt_mem [0:BR_CKPT_DEPTH-1][0:FL_DEPTH-1];
    reg [FL_IDX_W:0]            ckpt_head [0:BR_CKPT_DEPTH-1];
    reg [FL_IDX_W:0]            ckpt_tail [0:BR_CKPT_DEPTH-1];
    reg [BR_CKPT_IDX_W-1:0]     branch_ckpt_restore_slot;

    localparam [FL_IDX_W:0] FL_DEPTH_COUNT = FL_DEPTH[FL_IDX_W:0];
    localparam [FL_IDX_W-1:0] FL_LAST_IDX = (FL_DEPTH - 1);

    function [FL_IDX_W-1:0] ptr_index;
        input [FL_IDX_W:0] ptr;
        reg   [FL_IDX_W:0] normalized;
        begin
            normalized = ptr;
            if (normalized >= FL_DEPTH_COUNT)
                normalized = normalized - FL_DEPTH_COUNT;
            if (normalized >= FL_DEPTH_COUNT)
                normalized = normalized - FL_DEPTH_COUNT;
            ptr_index = normalized[FL_IDX_W-1:0];
        end
    endfunction

    function [FL_IDX_W-1:0] idx_inc;
        input [FL_IDX_W-1:0] idx;
        begin
            if (idx == FL_LAST_IDX)
                idx_inc = {FL_IDX_W{1'b0}};
            else
                idx_inc = idx + {{(FL_IDX_W-1){1'b0}}, 1'b1};
        end
    endfunction

    wire [FL_IDX_W:0] count;
    assign count = fl_tail - fl_head;

    wire [FL_IDX_W-1:0] head0_idx = ptr_index(fl_head);
    wire [FL_IDX_W-1:0] head1_idx = idx_inc(head0_idx);
    wire [1:0]          alloc_count = {1'b0, alloc0_req} + {1'b0, alloc1_req};

    assign alloc0_prd = fl_mem[head0_idx];
    assign alloc1_prd = fl_mem[alloc1_after_alloc0 ? head1_idx : head0_idx];
    assign can_alloc_1 = (count >= 1) && (alloc0_prd != {PHYS_REG_W{1'b0}});
    assign can_alloc_2 = (count >= 2) &&
                         (alloc0_prd != {PHYS_REG_W{1'b0}}) &&
                         (alloc1_prd != {PHYS_REG_W{1'b0}});

    integer find_idx;
    always @(*) begin
        branch_ckpt_restore_hit  = 1'b0;
        branch_ckpt_restore_slot = {BR_CKPT_IDX_W{1'b0}};
        if (branch_ckpt_restore) begin
            for (find_idx = 0; find_idx < BR_CKPT_DEPTH; find_idx = find_idx + 1) begin
                if (ckpt_valid[find_idx] &&
                    (ckpt_order_id[find_idx] == branch_ckpt_restore_order_id)) begin
                    branch_ckpt_restore_hit  = 1'b1;
                    branch_ckpt_restore_slot = find_idx[BR_CKPT_IDX_W-1:0];
                end
            end
        end
    end

    integer r, c, cap0_slot, cap1_slot, phys_idx, rebuild_tail;
    reg [FL_IDX_W:0] free_tail_after;
    reg [FL_IDX_W:0] cap_head_value;
    reg [FL_IDX_W:0] cap_tail_value;

    always @(posedge clk or negedge rstn) begin
        if (!rstn) begin
            fl_head <= {(FL_IDX_W+1){1'b0}};
            fl_tail <= NUM_FREE[FL_IDX_W:0];
            for (r = 0; r < FL_DEPTH; r = r + 1)
                fl_mem[r] <= (r < NUM_FREE) ? (32 + r) : {PHYS_REG_W{1'b0}};
            for (c = 0; c < BR_CKPT_DEPTH; c = c + 1) begin
                ckpt_valid[c] <= 1'b0;
                ckpt_order_id[c] <= {`METADATA_ORDER_ID_W{1'b0}};
                ckpt_head[c] <= {(FL_IDX_W+1){1'b0}};
                ckpt_tail[c] <= NUM_FREE[FL_IDX_W:0];
                for (r = 0; r < FL_DEPTH; r = r + 1)
                    ckpt_mem[c][r] <= (r < NUM_FREE) ? (32 + r) : {PHYS_REG_W{1'b0}};
            end
        end else begin
            if (reset_list) begin
                fl_head <= {(FL_IDX_W+1){1'b0}};
                fl_tail <= NUM_FREE[FL_IDX_W:0];
                for (r = 0; r < FL_DEPTH; r = r + 1)
                    fl_mem[r] <= (r < NUM_FREE) ? (32 + r) : {PHYS_REG_W{1'b0}};
                for (c = 0; c < BR_CKPT_DEPTH; c = c + 1)
                    ckpt_valid[c] <= 1'b0;
            end else if (rebuild_valid) begin
                fl_head <= {(FL_IDX_W+1){1'b0}};
                rebuild_tail = 0;
                for (r = 0; r < FL_DEPTH; r = r + 1)
                    fl_mem[r] <= {PHYS_REG_W{1'b0}};
                for (phys_idx = 1; phys_idx < FL_DEPTH; phys_idx = phys_idx + 1) begin
                    if (!rebuild_mapped_mask[phys_idx] && rebuild_tail < FL_DEPTH) begin
                        fl_mem[rebuild_tail] <= phys_idx[PHYS_REG_W-1:0];
                        rebuild_tail = rebuild_tail + 1;
                    end
                end
                fl_tail <= rebuild_tail[FL_IDX_W:0];
                for (c = 0; c < BR_CKPT_DEPTH; c = c + 1)
                    ckpt_valid[c] <= 1'b0;
            end else if (branch_ckpt_restore && branch_ckpt_restore_hit) begin
                fl_head <= ckpt_head[branch_ckpt_restore_slot];
                fl_tail <= ckpt_tail[branch_ckpt_restore_slot];
                for (r = 0; r < FL_DEPTH; r = r + 1)
                    fl_mem[r] <= ckpt_mem[branch_ckpt_restore_slot][r];
                for (c = 0; c < BR_CKPT_DEPTH; c = c + 1) begin
                    if (ckpt_valid[c] &&
                        (ckpt_order_id[c] >= branch_ckpt_restore_order_id))
                        ckpt_valid[c] <= 1'b0;
                end
            end else begin
                free_tail_after = fl_tail;

                if (alloc_count != 2'd0)
                    fl_head <= fl_head + alloc_count;

                if (free0_valid && free0_prd != {PHYS_REG_W{1'b0}}) begin
                    fl_mem[ptr_index(fl_tail)] <= free0_prd;
                    fl_tail <= fl_tail + 1'b1;
                    free_tail_after = fl_tail + 1'b1;
                end
                if (free1_valid && free1_prd != {PHYS_REG_W{1'b0}}) begin
                    if (free0_valid && free0_prd != {PHYS_REG_W{1'b0}}) begin
                        fl_mem[idx_inc(ptr_index(fl_tail))] <= free1_prd;
                        fl_tail <= fl_tail + 2'd2;
                        free_tail_after = fl_tail + 2'd2;
                    end else begin
                        fl_mem[ptr_index(fl_tail)] <= free1_prd;
                        fl_tail <= fl_tail + 1'b1;
                        free_tail_after = fl_tail + 1'b1;
                    end
                end

                if (recover_push_valid && recover_push_prd != {PHYS_REG_W{1'b0}}) begin
                    fl_mem[ptr_index(fl_tail)] <= recover_push_prd;
                    fl_tail <= fl_tail + 1'b1;
                end

                // Checkpoint freelists are immutable after capture.  This
                // keeps the freelist image in exact lockstep with the immutable
                // RMT image and avoids restoring a PRF as free while the RMT
                // snapshot still names it.  Commit frees after the checkpoint
                // remain available on the live path; a later mispredict may
                // conservatively lose those frees until a future full reset.

                if (branch_ckpt_drop0_valid) begin
                    for (c = 0; c < BR_CKPT_DEPTH; c = c + 1) begin
                        if (ckpt_valid[c] &&
                            (ckpt_order_id[c] <= branch_ckpt_drop0_order_id))
                            ckpt_valid[c] <= 1'b0;
                    end
                end
                if (branch_ckpt_drop1_valid) begin
                    for (c = 0; c < BR_CKPT_DEPTH; c = c + 1) begin
                        if (ckpt_valid[c] &&
                            (ckpt_order_id[c] <= branch_ckpt_drop1_order_id))
                            ckpt_valid[c] <= 1'b0;
                    end
                end

                cap0_slot = -1;
                if (branch_ckpt_capture0_valid) begin
                    for (c = 0; c < BR_CKPT_DEPTH; c = c + 1) begin
                        if (!ckpt_valid[c] && cap0_slot < 0)
                            cap0_slot = c;
                    end
                    if (cap0_slot >= 0) begin
                        ckpt_valid[cap0_slot] <= 1'b1;
                        ckpt_order_id[cap0_slot] <= branch_ckpt_capture0_order_id;
                        cap_head_value = fl_head + branch_ckpt_capture0_alloc_count;
                        cap_tail_value = fl_tail;
                        ckpt_head[cap0_slot] <= cap_head_value;
                        ckpt_tail[cap0_slot] <= cap_tail_value;
                        for (r = 0; r < FL_DEPTH; r = r + 1)
                            ckpt_mem[cap0_slot][r] <= fl_mem[r];
                    end
                end

                cap1_slot = -1;
                if (branch_ckpt_capture1_valid) begin
                    for (c = 0; c < BR_CKPT_DEPTH; c = c + 1) begin
                        if (!ckpt_valid[c] &&
                            !(branch_ckpt_capture0_valid && (cap0_slot == c)) &&
                            cap1_slot < 0)
                            cap1_slot = c;
                    end
                    if (cap1_slot >= 0) begin
                        ckpt_valid[cap1_slot] <= 1'b1;
                        ckpt_order_id[cap1_slot] <= branch_ckpt_capture1_order_id;
                        cap_head_value = fl_head + branch_ckpt_capture1_alloc_count;
                        cap_tail_value = fl_tail;
                        ckpt_head[cap1_slot] <= cap_head_value;
                        ckpt_tail[cap1_slot] <= cap_tail_value;
                        for (r = 0; r < FL_DEPTH; r = r + 1)
                            ckpt_mem[cap1_slot][r] <= fl_mem[r];
                    end
                end
            end
        end
    end

    // synthesis translate_off
    always @(posedge clk) begin
        if (rstn) begin
            if (alloc0_req && !can_alloc_1)
                $display("ERROR: freelist alloc0 underflow at %0t", $time);
            if (alloc0_req && alloc1_req && !can_alloc_2)
                $display("ERROR: freelist alloc1 underflow at %0t", $time);
            if (alloc0_req && (alloc0_prd == {PHYS_REG_W{1'b0}}))
                $display("ERROR: freelist alloc0 returned p0 at %0t", $time);
            if (alloc1_req && (alloc1_prd == {PHYS_REG_W{1'b0}}))
                $display("ERROR: freelist alloc1 returned p0 at %0t", $time);
        end
    end
    // synthesis translate_on

endmodule
