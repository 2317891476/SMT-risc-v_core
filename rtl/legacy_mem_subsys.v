...
wire uart_store_accept = sb_write_valid && sb_addr_is_uart_tx && !uart_pending_valid_r;
...
assign sb_write_ready =
    !sb_write_valid ? 1'b1 :
    sb_addr_is_uart_tx ? !uart_pending_valid_r :
    ...
    if (!rstn) begin
        ...
        if (load_req) begin
            if (load_addr_is_ram) begin
                ...
                // Report both the active frame and the one-byte pending queue.
                // Software polling this register should only observe "idle"
                // once the UART can safely accept the next byte end-to-end.
                load_rdata <= {31'd0, uart_status_busy};
