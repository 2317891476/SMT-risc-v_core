...
module tb_ax7203_branch_probe_smoke;
    reg sys_clk_p;
    reg sys_clk_n;
    reg sys_rst_n;
    reg uart_rx;
    ...
    initial begin : timeout_guard
        #TB_TIMEOUT_NS;
        $display("[AX7203_BRANCH_PROBE] TIMEOUT ready=%0b retire=%0b tube=%0b uart_edges=%0d led=%b",
    ...
    always @(posedge sys_clk_p) begin
        if (sys_rst_n && ready_seen && retire_seen && tube_seen && uart_edge_count > 8) begin
            $display("[AX7203_BRANCH_PROBE] PASS ready=%0b retire=%0b tube=%0b uart_edges=%0d led=%b",
                     ready_seen, retire_seen, tube_seen, uart_edge_count, led);
            $finish;
        end
    end
