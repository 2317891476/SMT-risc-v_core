...
- Current Linux blockers: I/D translation, the `mem_subsys` PTW endpoint, commit-ordered full-flush `sfence.vma`, precise ROB-commit fetch/load/store/branch-target exceptions, MPRV effective privilege, and board-baseline PTW A/D cache visibility are integrated. Remaining blockers are full `fw_payload.bin` production, then OpenSBI/Linux token bring-up under `linux-preload`.
- Verification on 2026-05-26 after the `rv32si` architecture fixes: `test_sv32_translation`, `test_sv32_page_faults`, and `test_sfence_vma` pass 3/3 under `--fpga-config`; Linux directed tests including `test_sv32_core_identity`, `test_sv32_core_page_fault`, and `test_sv32_core_fetch_page_fault` pass 7/7 under `--fpga-config`; `--basic` and `--basic --fpga-config` both pass 44/44; `rv32ua` riscv-tests pass 10/10; `rv32si` riscv-tests pass 6/6.
- `rv32si` status: all six supervisor tests now pass. `dirty` exposed two MMU issues: D-side MPRV must use `mstatus.MPP` as the effective privilege when current mode is M and `MPRV=1`, and PTW permission checks must snapshot effective privilege/SUM/MXR so a younger CSR write cannot change an older in-flight walk. `ma_fetch` exposed instruction-address-misaligned branch/jump exceptions, JALR bit-0 clearing, and precise recovery of the faulting instruction's destination mapping.
- Board-equivalent WSL Verilator Dhrystone after the `rv32si` fixes passes in both 5000-run modes: preload has `BoardConfigMatch=True`, `PreloadBenchmarkPass=True`, no trap/stuck/unexpected UART, and `Cycles=28837573`; loader-semantic has `BoardConfigMatch=True`, `LoaderSemanticPass=True`, 128 block ACKs, `DHRYSTONE DONE`, no trap/stuck/unexpected UART, and `Cycles=166376414`.
...
- 2026-05-26: `rv32si` now passes 6/6. Fixes covered MPRV effective D-side privilege, PTW snapshot/latching for permission faults, JALR bit-0 clearing, instruction-address-misaligned exceptions for non-C taken control transfers, precise ROB-commit pipe0 exception delivery, and recovery of the faulting instruction's destination mapping.
