...
- Classic `riscv-tests` supervisor coverage is now usable as an architecture-gap detector. `python verification\run_riscv_tests.py --suite riscv-tests --categories rv32si` builds all six tests after adding `zicsr` to `-march`, local supervisor adapter constants/macros, and a correct MPP-to-S entry sequence. Current result is 4/6: `csr`, `sbreak`, `scall`, and `wfi` pass; `dirty` and `ma_fetch` fail.
...
- Remaining `rv32si` failures are no longer build-harness failures. `dirty` points at MPRV effective-privilege Sv32 translation and hardware A/D behavior under machine-mode access; `ma_fetch` points at instruction-address-misaligned exception generation/delegation around halfword targets in a non-C core.
...
- After precise fetch/load/store page-fault integration and PTW A/D snoop-invalidate wiring, board-equivalent 5000-run Verilator Dhrystone still passes in both preload and loader-semantic modes. Preload reported `BoardConfigMatch=True`, `PreloadBenchmarkPass=True`, `Cycles=28837429`, no trap, no unexpected UART, and no stuck PC. Loader-semantic reported `BoardConfigMatch=True`, `LoaderSemanticPass=True`, 128 block ACKs, `Cycles=166376270`, benchmark START/DONE, no trap, no unexpected UART, and no stuck PC.
