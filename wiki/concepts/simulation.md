...
- Classic `riscv-tests` supervisor coverage is now usable and currently green. `python verification\run_riscv_tests.py --suite riscv-tests --categories rv32si` builds all six tests after adding `zicsr` to `-march`, local supervisor adapter constants/macros, and a correct MPP-to-S entry sequence. Current result is 6/6 after fixing the `dirty` and `ma_fetch` architecture gaps.
- The `rv32si` adapter finding on 2026-05-26: entering S-mode with `csrs mstatus, MSTATUS_MPP` was wrong because it left MPP as M. The adapter now clears MPP and sets `MPP=S`; M-mode tests also clear reset MPP so later MPRV setup can select S as the effective privilege.
- The `dirty` root causes were MPRV effective privilege and PTW replay precision. D-side translation now uses `mstatus.MPP` when current mode is M and `MPRV=1`; PTW permission checks snapshot effective privilege/SUM/MXR; and PTW faults are latched for the still-asserted request so the same faulting store cannot immediately replay under a younger `SUM=1` CSR write and set the PTE D bit.
- The `ma_fetch` root causes were missing instruction-address-misaligned exceptions for non-C taken control transfers, missing JALR bit-0 clearing, and imprecise pipe0 exception retirement. Pipe0 synchronous exceptions now record by RS tag and enter CSR only at ROB commit, and exception commit restores the faulting instruction's rd mapping/frees its new physical register instead of retiring the destination.
...
- After the `rv32si` fixes, board-equivalent 5000-run Verilator Dhrystone still passes in both preload and loader-semantic modes. Preload reported `BoardConfigMatch=True`, `PreloadBenchmarkPass=True`, `Cycles=28837573`, no trap, no unexpected UART, and no stuck PC. Loader-semantic reported `BoardConfigMatch=True`, `LoaderSemanticPass=True`, 128 block ACKs, `Cycles=166376414`, benchmark START/DONE, no trap, no unexpected UART, and no stuck PC.
...
As of 2026-05-26, the mode is wired into the wrapper/harness but cannot pass because `build\linux\fw_payload.bin` is not produced yet. Precise fetch/load/store page faults now reach CSR at ROB commit, and the board-baseline PTW A/D cache visibility path is covered by `test_sv32_core_identity`. The command correctly fails early with a missing-payload message when the image is absent.
