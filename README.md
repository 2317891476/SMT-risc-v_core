...
| **分支预测** | 1024-entry 双模态 (Bimodal) 2-bit 饱和计数器 + BTB + 8-entry/thread RAS |
...
> **当前已验证 AX7203 主基线**: OoO 后端 + `FPGA_MODE=1` + `ENABLE_MEM_SUBSYS=1` + `ENABLE_DDR3=1` + `L2_PASSTHROUGH=1` + `ENABLE_ROCC_ACCEL=0` + `SMT_MODE=1` + `RS_DEPTH=16` + `FetchBuffer=16` + `25MHz`。最新真实上板 bitstream 仍为 **2026-04-22** 的 `BuildID=0x69E89179`（JTAG 回读、DDR3 `DEADBEEF` 写回读和 UART 板测通过）。**2026-04-25 已完成 OoO + full L1DCache 的 Verilator / FPGA 同配置 / FPGA top-sim 级集成验证；本轮未重新跑真实 AX7203 上板。**

### 最新验证快照（2026-04-25）

| 项目 | 当前结果 | 说明 |
|------|------|------|
| 本地基础回归 | ✅ `35/35 PASS` | `python verification/run_all_tests.py --basic` |
| FPGA 同配置基础回归 | ✅ `35/35 PASS` | `python verification/run_all_tests.py --basic --fpga-config` |
| MMIO/投机防火墙定向回归 | ✅ `15/15 PASS` | 覆盖 wrong-path MMIO load/store、ROB-head MMIO load、older-store ordering、CSR/MRET/CLINT/PLIC |
| Verilator Dhrystone（完整 L1D, latency=1） | ✅ `ExitReason=done` | `Cycles=197,753`, `InstRetired=58,153`, `IPC_X1000=294`, `EffectiveRuns=10`，约 **19,775 cycles/run**，`SpecMmioLoadViolationCount=0` |
| Verilator Dhrystone（完整 L1D, latency=7） | ✅ `ExitReason=done` | `Cycles=239,532`, `InstRetired=58,153`, `IPC_X1000=242`, `EffectiveRuns=10`，约 **23,953 cycles/run**，`DCacheMissEventCount=3790`，`SpecMmioLoadViolationCount=0` |
| Verilator DCache 诊断模式 | ✅ 保留 | `--dcache-mode {full,passthrough,registered-pt,read-only}`；默认 `full`，`passthrough` 仅用于定位 OoO 控制链 |
...
### OoO + L1DCache 调试结论（2026-04-25）

- `mock-latency >= 7` 下的原始活锁根因不在 DCache，而在 OoO 控制链；当前 full L1D + latency=1/7 均已稳定 `ExitReason=done`。
- 微优化 A/B/C' 已保留：LSU hit response early wakeup、INT/MUL/DIV `WAKE_HOLD=0`（MEM 仍为 1）、ROB WB-head commit bypass。
- 投机安全边界已补齐：分支后只允许普通 ALU/MUL/DIV 安全投机；MEM/CSR/MRET/RoCC 继续被 unresolved branch 阻塞。
- MMIO load 防火墙已落地：UART/CLINT/PLIC/`0x1300_xxxx` load 必须到对应 ROB head 且没有同线程 older store pending 才能向 M1 发请求；Verilator 对 speculative MMIO load violation 做硬失败。
- Store 仍只在 ROB commit 后由 StoreBuffer drain；wrong-path store / MMIO store 由 flush/order 清除，保持单 side-effect / 单 LSU completion。
- 本轮另修一个 flush 同拍控制漏洞：被 flush kill 的 wrong-path branch issue 不再置 `branch_in_flight`，避免 ROB 已空但 `branch_pending` 永久卡住后续 MMIO load。
- L1DCache 最终路径保持完整 4KB 4-way write-back/write-allocate；只有 DDR3 `0x8000_0000+` 进入 DCache，MMIO 继续 uncached bypass。
...
```
  ...
  ║       ↓                                            ║
  ║  PC_MT ──→ IROM ──→ BPU(1024-entry Bimodal+BTB+RAS)║
  ║       ↓                                            ║
...
│  ├─ bpu_bimodal.v                 # ★ 1024-entry 分支预测器 + BTB + RAS
│  ├─ fetch_buffer.v                # ★ 16-entry 取指缓冲 FIFO（当前主线）
...
| `bpu_bimodal` | 2-bit 饱和计数器 + 直接映射 BTB + RAS | PHT_ENTRIES=1024, RAS_DEPTH=8, XOR-fold thread indexing |
| `fetch_buffer` | 取指缓冲 FIFO，双弹出支持双译码 | 主线 DEPTH=16, per-thread flush |
...
| `issue_queue` | 参数化发射队列：CDB 唤醒、oldest-first 选择、提交释放 | INT/MUL/DIV WAKE_HOLD=0, MEM WAKE_HOLD=1, DEALLOC_AT_COMMIT |
...
============================================================
  ...
  [PASS] test_branch_spec_mmio_load_flush: PASS
  [PASS] test_mmio_load_rob_head: PASS
  [PASS] test_mmio_load_store_order: PASS
...
当前 `--basic` 默认包含 **35** 个测试；其中包含 divider、StoreBuffer、branch speculation、MMIO firewall、L2/MMIO、CSR/CLINT/PLIC 回归。RoCC 的 3 个专项测试默认关闭，需要 `--enable-rocc` 才会纳入回归。`test_smt.s` 仍可通过 `--tests test_smt.s` 单独运行，但不在当前默认 basic 集合内。
...
| `test_branch_spec_alu_flush.s` | wrong-path ALU flush | 投机 ALU 结果不能更新架构状态 |
| `test_branch_spec_mem_sideeffect.s` | wrong-path store/MMIO side-effect | 分支错误路径不能产生 store/MMIO 副作用 |
| `test_branch_spec_system_block.s` | 分支后 system 指令阻塞 | CSR/MRET/RoCC 不允许在 unresolved branch 后越界 |
| `test_branch_spec_correct_path_alu.s` | correct-path ALU 安全投机 | 分支后普通 ALU 可提前执行并最终提交 |
| `test_branch_spec_mmio_load_flush.s` | wrong-path MMIO load flush | 错误路径 MMIO load 不产生 M1 读请求 |
| `test_mmio_load_rob_head.s` | MMIO load ROB-head firewall | MMIO load 到 ROB head 后才允许完成 |
| `test_mmio_load_store_order.s` | older store → younger MMIO load 顺序 | older store drain 先于 younger MMIO load |
...
**BPU 配置：**
- 1024-entry PHT (Pattern History Table)
- 2-bit 饱和计数器
- 直接映射 BTB (Branch Target Buffer)
- 8-entry/thread RAS，用于 call/return 预测
- 每线程独立索引 (XOR-fold)
...
| 执行单元 | **2× ALU + 1× MUL (3-cycle) + 1× DIV + AGU/LSU** |
| 分支预测 | **1024-entry Bimodal + BTB + RAS** |
| 前递网络 | **PRF 旁路** (tagbuf→PRF fallback；INT/MUL/DIV WAKE_HOLD=0，MEM WAKE_HOLD=1) |
| 缓存 | **4KB 4-way M1-native L1 DCache，DDR3 cached / MMIO uncached bypass** |
...
python fpga/scripts/run_verilator_mainline.py --mode preload --benchmark dhrystone --runs 1 --mock-latency 1 --dcache-mode full

# latency=7 / full L1D 门禁
python fpga/scripts/run_verilator_mainline.py --mode preload --benchmark dhrystone --runs 1 --mock-latency 7 --dcache-mode full
...
| 项目 | 当前结果 | 证据 |
|------|------|------|
| `preload --build-only` | ✅ 通过 | `build/verilator/mainline/preload/` |
| `loader-semantic --build-only` | ✅ 通过 | `build/verilator/mainline/loader-semantic/dhrystone_runs1/` |
| `preload --runs 1 --mock-latency 1 --dcache-mode full` | ✅ 通过，`ExitReason=done` | `build/verilator/mainline/preload/dhrystone_runs1_lat1_full/summary.txt` |
| `preload --runs 1 --mock-latency 7 --dcache-mode full` | ✅ 通过，`ExitReason=done` | `build/verilator/mainline/preload/dhrystone_runs1_lat7_full/summary.txt` |
...
- full L1D 模式下 `mock-latency=1` 与 `mock-latency=7` 当前都稳定结束于 `ExitReason=done`
- 当前 Verilator 下固定采用 `EffectiveRuns=10`，目标是让第二环境稳定走完 benchmark 完成路径，而不是在 Verilator 中复刻板级自校准跑分
- latency=1 当前结果：`Cycles=197,753`、`InstRetired=58,153`、`IPCx1000=294`，约 `19,775 cycles/run`
- latency=7 当前结果：`Cycles=239,532`、`InstRetired=58,153`、`IPCx1000=242`，约 `23,953 cycles/run`
- `SpecMmioLoadViolationCount=0` 是硬门禁；`SpecMmioLoadBlockedCycles` / `MmioLoadAtRobHeadCount` / `OlderStoreBlockedMmioLoadCycles` 用于 MMIO firewall 审计
...
- `build/verilator/mainline/preload/dhrystone_runs1_lat1_full/summary.txt`
- `build/verilator/mainline/preload/dhrystone_runs1_lat1_full/summary.json`
- `build/verilator/mainline/preload/dhrystone_runs1_lat7_full/summary.txt`
- `build/verilator/mainline/preload/dhrystone_runs1_lat7_full/summary.json`
- `build/verilator/mainline/loader-semantic/dhrystone_runs1/summary.txt`

> **说明**: 这套 Verilator 环境承担"高速验证 benchmark 主线路径、稳定输出 `DHRYSTONE DONE`"的快速回归任务。它不替代 §13.3 板级签核；§13.10 已记录的 AX7203 板级 Dhrystone 端到端跑分（最新 Dhrystone bitstream `BuildID=0x69E8D721`, IPC_X1000=180）仍是 DDR3 loader / FPGA 板测主线的真实门禁。
...
### 基础测试（截至 2026-04-25，本地 `--basic` 回归）

| 测试 | 结果 |
|------|---------|
| 核心功能测试 (`test1` / `test2` / `test_rv32i_full`) | ✅ PASS |
| **Divider 测试 (2个)** | ✅ PASS |
| **Branch speculation / MMIO firewall 测试 (7个)** | ✅ PASS |
| **Store Buffer 测试 (10个)** | ✅ PASS |
| **L2/Cache/MMIO 测试 (7个)** | ✅ PASS |
| **CSR/中断测试 (6个)** | ✅ PASS |
| **默认 basic 总计** | **35/35 PASS** |
...
| mhpmcounter5 | 0xB05 / 0xB85 | 0xC05 / 0xC85 | `dcache_miss` | 2026-04-22 板级 bitstream 中为占位；当前 Verilator full L1D 另由 `DCacheMissEventCount` 审计 |
...
BENCH ICACHE_MISS: <N>
BENCH DCACHE_MISS: <N>       <- 2026-04-22 板级 bitstream 中为占位
