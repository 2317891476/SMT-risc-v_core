...
cd .\comp_test
$env:PATH = "E:\iverilog\bin;E:\xpack-riscv-none-elf-gcc-15.2.0-1\bin;" + $env:PATH
.\run_iverilog_tests.ps1 -Flow -Tests @("test1.s","test2.S","test_smt.s") -NoGtkWave
```

`run_iverilog_tests.ps1 -Flow V2` 是 AX7203 / 仿真的唯一规范入口；它会显式解析 `tb_v2.sv`、`module_list_v2` 和 `..\rtl\`，不再文档化裸 `iverilog` 命令。
...
| 特性 | V1 (adam_riscv.v) | (adam_riscv.v) |
|------|-------------------|----------------------|
...
**Q3: V1 和 可以并存吗？**
是的。V1 (`adam_riscv.v` + `tb.sv` + `module_list`) 和 (`adam_riscv.v` + `tb_v2.sv` + `module_list_v2`) 完全独立，互不影响。

**Q4: 为什么 管线还用 stage_mem / data_memory？**
当前 的存储路径在仿真中直接复用 V1 的 `stage_mem` + `data_memory` (SRAM 行为模型)，确保功能正确。`l1_dcache_nb` 和 `mmu_sv32` 已就位，待切换到 AXI4 仿真环境后启用。
...
V2 编译/运行统一由 `comp_test/run_iverilog_tests.ps1 -Flow ...` 处理；测试差异通过 `-Tests` 选择，不再维护单独的裸 `iverilog` 编译命令。
